/*
* We want to bin traces that we emit so that developers don't have lots of 
* traces for the same bug. This file implements a fuzzy stack that we use to 
* determine if two call stacks are similar.
*/
"use strict;"

//Disable trace handling inside trace handler to avoid recursion
//  First thing is set to true to prevent any action taken till loading is complete -- set to false at end
...
/*
*Check and emit time travel trace -- also invoke remoteTraceManagerObj functions as needed.
...
function emitTrace_helper(emitKind, optInfo) {
    ...
    try {
        ...
        //Process a synchronous write action for a unique trace bin -- otherwise a async action on a multiple trace bin
        var sampleRes = buildTraceResult('no-sample');
        if (emitKind === 'emitOnExit' || emitKind === 'emitOnException' || emitKind === 'emitOnSigInt') {
...
function emitSyncTraceKind(emitKind, optInfo) {
    ...
    var action = function () {
        try {
            process.stderr.write(`    Write error trace to: ${resolvedTracePath}\n`);
            global.emitTTDLog(resolvedTracePath);

            if(emitOptions.remoteTraceManagerObj && emitOptions.remoteTraceManagerObj.uploadTraceSync) {
...
function emitAsyncTraceKind(emitKind) {
    ...
    var traceName = traceBucketName + path.sep + traceDirName + path.sep;
    ...
    var action = function () {
        try {
            ...
            if(emitOptions.remoteTraceManagerObj && emitOptions.remoteTraceManagerObj.uploadTraceAsync) {
...
function createTraceLogTarget(tracename) {
    ...
    if (traceRootDir === undefined) {
        traceRootDir = path.dirname(process.mainModule.filename);
    }
    resolvedTracePath = path.resolve(traceRootDir, '_diagnosticTraces', tracename);
...
function ensureTraceTarget(pth) {
    //I don't like this and don't want it to be happening here so I am going to bail
...
function createTargetDirectory(pth) {
    ...
    try {
        fs.accessSync(pth, fs.constants.R_OK | fs.constants.W_OK | fs.constants.X_OK);
        return true;
    }
    catch (ei) { ; }
    ...
    do {
        ...
        if (prefixPath === path.dirname(prefixPath)) {
            process.stderr.write(`Failed creating trace directory (bad prefix): ${pth} -> ${prefixPath}\n`);
            return false;
        }
        ...
        try {
            fs.accessSync(prefixPath, fs.constants.R_OK | fs.constants.W_OK | fs.constants.X_OK);
            baseFound = true;
        }
        catch (ei) { ; }
    } while (!baseFound);
    ...
    while (suffixPaths.length > 0) {
        try {
            ...
            process.stderr.write(`Failed creating trace directory (failed mkdir): ${ec}\n`);
            return false;
        }
    }
    ...
    try {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (stats.isFile()) {
                ...
                deleteTargetDirectoryContents(fpath);
                fs.rmdirSync(fpath);
            }
            else {
...
function updateGlobalSampleStats(eventKind) {
    currentSampleRate[eventKind] *= currentSampleRate[eventKind] * emitOptions.backoffFactors[eventKind];
...
var emitOptions = {
    ...
    globalBackoffCancelInterval: 5000, //time at which we reset the backoff interval

    //The probability that we sample a trace on the first warn/error/assert ever encountered
    initialRates: {
        emitOnLogWarn: 0.25,
        emitOnLogError: 0.25,
        emitOnAssert: 1.0
    ...
    binBackoffFactor: 0.5, //sample with probability #preserved * binBbackoffFactor

    localTraceDirectory: undefined, //The local directory that is the root for storing traces
...
var emitMinTimeValue = { emitOnLogWarn: new Date(2009, 8, 12), emitOnLogError: new Date(2009, 8, 12), emitOnAssert: new Date(2009, 8, 12) };
...
function setOptions(optionsObj) {
    for (var opt in optionsObj) {
        //TODO: we should do more error checking on the validity of the option values we get from the user
...
function checkGlobalShouldEmit(emitKind, optInfo) {
    ...
    if (emitKind === 'emitOnExit') {
        ...
        if (sampleInterval >= emitOptions.globalBackoffCancelInterval) {
            currentSampleRate[emitKind] = Math.max(currentSampleRate[emitKind], emitOptions.baselineRates[emitKind]);
        }

        //console.log('Sampling ' + emitKind + ' at rate: ' + currentSampleRate[emitKind]);
...
/*
*Given a fuzzy stack see if we want to write a trace for the call (looking at the error bin).
...
function checkBinShouldEmit(fuzzyStack) {
    ...
    //stop sampling after max sampled values -- e.g. we don't need 100 repros of the same issue
    ...
    //console.log("Sample bib with prob: " + sampleProb);
...
var directIsAbsolute = (process.platform === 'win32') ? directIsAbsoluteW32 : directIsAbsolutePosix;
...
function generateFuzzyStack(eventKind) {
    ...
    var fname = lastframe.substr(lastframe.lastIndexOf(path.sep) + 1).replace('.js:', '_line-').replace(':', '_column-');
